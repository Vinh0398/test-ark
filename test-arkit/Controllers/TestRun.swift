...
class TestRun {
    ...
    func getSizeDetectedObject(_ objectAnchor: ARObjectAnchor) -> String{
        let width = objectAnchor.referenceObject.extent.x
        let height = objectAnchor.referenceObject.extent.y
        let length = objectAnchor.referenceObject.extent.z
        let volume = width * height * length
        return "width \(width) " + "height \(height) " + "length \(length) " + "volume object \(volume) "
    }
